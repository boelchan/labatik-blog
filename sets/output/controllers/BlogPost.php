...
class BlogPost extends MY_Controller
{
    ...
    public function getDatatable()
    {
        ...
        if ( $t['rows'] ) 
        {
            foreach ($t['rows'] as $d) 
            {
                ...
                $records["data"][] = array(
                    $checkbox,
					btnRead($id, '<a href="'. load_image("blog_post_banner/".$d->banner_img) .'" target="_blank">
					...
					$d->judul_seo,
					$d->judul,
					$d->{$this->blogKategoriModel->label},
					$d->konten,
					$d->status,
					$d->status_date,
                    btnEditTable($id)
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('banner_img','banner img','trim');
		...
		if(!empty($this->input->post('status_date')))
        $this->form_validation->set_rules('status_date','status date','trim|callback_check_date_format');
        ...
        else 
        {
            $data = array(
                        ...
                        'status_date' => sys_date($this->input->post('status_date')),
                    );
