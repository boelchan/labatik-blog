...
class Harviacode
{
    ...
    function reference_field($table)
    {
        ...
        $stmt->execute();
        while ($stmt->fetch()) {
            $rt = explode('_', $referenced_table_name);
            $ra = array();
            foreach ( $rt as $r)
            {
                $ra[] = ucfirst($r);
            }
            $referenced_table = implode('', $ra);
            $fields[$column_name] = array(
                'column_name' => $column_name, 
                'r_table_ori' => $referenced_table_name, 
                'r_table' => $referenced_table, 
                'r_column' => $referenced_column_name,
                'r_model' => $referenced_table.'Model'
            );
        }
    ...
    public function with_reference($all,$reference)
    {
        foreach ($all as $key => $row) {
            ...
            if(array_key_exists($row["column_name"], $reference) ) {
                ...
                $all[$key]['r_label']='$this->'.lcfirst($reference[$row["column_name"]]["r_model"]).'->label';
