...
class Ion_auth_model extends CI_Model
{
    ...
    public function register($identity, $password, $email, $additional_data = [], $groups = [])
    {
        ...
        $query = $this->db->get_where($this->tables['groups'], ['id' => $this->config->item('default_group', 'ion_auth')], 1)->row();
    ...
    public function user($id = null)
    {
        ...
        $id = isset($id) ? $id : $this->jwt->user_id;
    ...
    public function get_users_groups($id = false)
    {
        ...
        // return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description, , ' . $this->tables['users_groups'] . '.wilayah_id')
    ...
    public function set_session($user)
    {
        ...
        // $session_data['exp'] 	= strtotime('+5 hour');
        // $session_data['group_id'] = $this->get_users_groups($user->id)->row()->id;
        // $session_data['wilayah_id'] = $this->ion_auth->get_users_groups($user->id)->row()->wilayah_id;
        $res['token'] 	= JWT::encode($session_data, $this->config->item('jwt_key'));

        $this->session->set_userdata($this->config->item('jwt_key'),$res['token']);
    ...
    public function login_remembered_user()
    {
        ...
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, remember_code, last_login, first_name')
