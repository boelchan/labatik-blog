...
class Auth extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // $this->load->helper(['url', 'language','jwt']);
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            $this->_render_page('auth' . DIRECTORY_SEPARATOR . 'v_login', $this->data);
        }
    }
    ...
    public function login_action()
    {
        ...
        $this->form_validation->set_rules('identity', 'Indentity', 'trim|required'); //valid_email
        ...
        if ($this->form_validation->run() === true) {
            ...
            // check for "remember me"
            $remember = (bool) $this->input->post('remember');
            // $w = array('identity' => $this->input->post('identity') );

            if ($user_id = $this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                //if the login is successful
                //redirect them back to the home page
                $res['success'] = true;
                $res['message'] = $this->ion_auth->messages();


                $group_id = $this->ion_auth->get_users_groups($user_id)->row()->id;

                if ($group_id == 10) {
                    $res['url'] = site_url('profile');
                } else {
                ...
                // redirect them back to the login page
                $res['success'] = false;
                $res['message'] = $this->ion_auth->errors();
            }
        } else {
    ...
    public function forgot_password()
    {
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($forgotten) {
                ...
                $this->email->clear();
                $this->email->from('no-reply@labatik.id', $this->config->item('site_title', 'ion_auth'));
    ...
    public function create_user()
    {
        ...
        $this->form_validation->set_rules('phone', 'No Hp', 'trim|required|is_unique['.$tables['users'].'.phone]');
        ...
        if ($this->form_validation->run() === true ) {
            ...
            if ($a){
                $res['success'] = true;
                $res['message'] = 'Behasil';
                $res['url'] = site_url('masuk');
            }
        } else {
            $res['message'] = 'Lengkapi isian dengan benar';
            $res['field_error'] = $this->form_validation->error_array();
        }
        $this->output->set_content_type('application/json')->set_output(json_encode($res));
    }
    ...
    public function edit_user($id)
    {
        ...
        //     $is_unique_m =  '|is_unique[users.email]';
        // } else {
        //     $is_unique_m =  '';
        // }

        if($this->input->post('phone') != $row->phone) {
            $is_unique_p =  '|is_unique[users.phone]';
        } else {
            $is_unique_p =  '';
        }
        ...
        // $this->form_validation->set_message('is_unique', 'data sudah dipakai');
        $this->form_validation->set_rules('phone', '', 'trim|required'.$is_unique_p);
